...
class VCDException : public std::exception {
 public:
  explicit VCDException(const std::string &message)
      : m_message("VCD error: " + message) {}
...
class VCDPhaseException : public VCDException {
 public:
  explicit VCDPhaseException(const std::string &message)
      : VCDException(message) {}
};

class VCDTypeException : public VCDException {
 public:
  explicit VCDTypeException(const std::string &message)
      : VCDException(message) {}
};
...
class VCDWriter {
 ...
 public:
  VCDWriter(const std::string &filename, HeadPtr &header,
            unsigned init_timestamp = 0u);
  ...
  // Note, *size* may be `0`, some types ("int", "real", "event") have a default
  // size
  VarPtr register_var(
      const std::string
          &scope,  // Variable belongs within the hierarchical scope
  ...
  //         *false* if new_value is not changed from priveios *timestamp* for a
  //         given var
  bool change(VarPtr var, TimeStamp timestamp, const VarValue &value) {
    return _change(var, timestamp, value, false);
  }

  bool change(const std::string &scope, const std::string &name,
  ...
  // After `close()`, NO variable registration or value changes will be
  // accepted. Note, the output file-stream will be closed in destructor of
  // `VCDWriter`
 ...
 protected:
  ...
  void _scope_declaration(const std::string &scope, size_t sub_beg,
                          size_t sub_end = std::string::npos);
