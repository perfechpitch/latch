...
struct VCDHeader final {
  ...
  VCDHeader(TimeScale timescaleQuan, TimeScaleUnit timescaleUnit,
            const std::string &date, const std::string &comment,
            const std::string &version)
      : timescale_quan{timescaleQuan},
  ...
  std::string timescale() const {
    ...
    return std::to_string(int(timescale_quan)) + " " +
           TIMESCALE_UNITS[int(timescale_unit)];
  }
};

// -----------------------------
HeadPtr makeVCDHeader(TimeScale timescale_quan, TimeScaleUnit timescale_unit,
                      const std::string &date, const std::string &comment,
                      const std::string &version) {
  return HeadPtr{
      new VCDHeader(timescale_quan, timescale_unit, date, comment, version)};
}

// -----------------------------
const std::string VCDHeader::kw_names[VCDHeader::kws] = {
    "$timescale", "$date", "$comment", "$version"};
...
struct VCDScope final {
  ...
  VCDScope(const std::string &scopeName, ScopeType scopeType)
...
class VCDVariable {
 ...
 protected:
  VCDVariable(const std::string &name, VariableType type, unsigned size,
              ScopePtr scope, unsigned next_var_id);
...
const std::string VCDVariable::VAR_TYPES[] = {
    "wire",   "reg",   "string",  "parameter", "integer", "real",   "realtime",
...
struct VCDScalarVariable : public VCDVariable {
  VCDScalarVariable(const std::string &name, VariableType type, unsigned size,
                    ScopePtr scope, unsigned next_var_id)
...
struct VCDStringVariable : public VCDVariable {
  VCDStringVariable(const std::string &name, VariableType type, unsigned size,
                    ScopePtr scope, unsigned next_var_id)
      : VCDVariable(name, type, size, scope, next_var_id) {}
  VarValue change_record(const VarValue &value) const {
    if (value.find(' ') != std::string::npos)
      throw VCDTypeException{
          format("Invalid string value '%s'", value.c_str())};
...
struct VCDRealVariable : public VCDVariable {
  VCDRealVariable(const std::string &name, VariableType type, unsigned size,
                  ScopePtr scope, unsigned next_var_id)
      : VCDVariable(name, type, size, scope, next_var_id) {}
...
struct VCDVectorVariable : public VCDVariable {
  VCDVectorVariable(const std::string &name, VariableType type, unsigned size,
                    ScopePtr scope, unsigned next_var_id)
      : VCDVariable(name, type, size, scope, next_var_id) {}
  std::string change_record(const VarValue &value) const;
};
...
// -----------------------------
VCDWriter::VCDWriter(const std::string &filename, HeadPtr &header,
                     unsigned init_timestamp)
    : _filename(filename),
  ...
  if (!_header) throw VCDTypeException{"Invalid pointer to header"};

  _ofile = std::fopen(_filename.c_str(), "w");
  if (!_ofile)
    throw VCDTypeException{
        format("Can't open file '%s' for writing", _filename.c_str())};
}

// -----------------------------
VarPtr VCDWriter::register_var(const std::string &scope,
                               const std::string &name, VariableType type,
                               unsigned size, const VarValue &init,
                               bool duplicate_names_check) {
  VarPtr pvar;
  if (_closed) throw VCDPhaseException{"Cannot register after close()"};
  if (!_registering)
    throw VCDPhaseException{format(
        "Cannot register new var '%s', registering finished", name.c_str())};

  if (scope.size() == 0 || name.size() == 0)
    throw VCDTypeException{
        format("Empty scope '%s' or name '%s'", scope.c_str(), name.c_str())};

  _search->vcd_scope.name = scope;
  auto cur_scope = _scopes.find(_search->ptr_scope);
  if (cur_scope == _scopes.end()) {
    auto res =
        _scopes.insert(std::make_shared<VCDScope>(scope, _scope_def_type));
    if (!res.second)
      throw VCDPhaseException{
          format("Cannot insert scope '%s'", scope.c_str())};
    cur_scope = res.first;
  }
  ...
  switch (type) {
    ...
    case VariableType::realtime:
      if (sz(64) == 1)
        pvar = VarPtr(
            new VCDScalarVariable(name, type, 1, *cur_scope, _next_var_id));
      else
        pvar = VarPtr(new VCDVectorVariable(name, type, sz(64), *cur_scope,
                                            _next_var_id));
      break;

    case VariableType::real:
      pvar = VarPtr(
          new VCDRealVariable(name, type, sz(64), *cur_scope, _next_var_id));
    ...
    case VariableType::string:
      pvar = VarPtr(
          new VCDStringVariable(name, type, sz(1), *cur_scope, _next_var_id));
      break;
    ...
    default:
      if (!size)
        throw VCDTypeException{
            format("Must supply size for type '%s' of var '%s'",
                   VCDVariable::VAR_TYPES[(int)type].c_str(), name.c_str())};

      pvar = VarPtr(
          new VCDVectorVariable(name, type, size, *cur_scope, _next_var_id));
      if (init_value.size() == 1 && init_value[0] == VCDValues::UNDEF)
        init_value = std::string(size, VCDValues::UNDEF);
      break;
  }
  if (type != VariableType::event) _change(pvar, _timestamp, init_value, true);

  if (duplicate_names_check && _vars.find(pvar) != _vars.end())
    throw VCDTypeException{format("Duplicate var '%s' in scope '%s'",
                                  name.c_str(), scope.c_str())};
...
// -----------------------------
bool VCDWriter::_change(VarPtr var, TimeStamp timestamp, const VarValue &value,
                        bool reg) {
  if (timestamp < _timestamp)
    throw VCDPhaseException{
        format("Out of order value change var '%s'", var->_name.c_str())};
  if (_closed) throw VCDPhaseException{"Cannot change value after close()"};

  if (!var) throw VCDTypeException{"Invalid VCDVariable"};

  if (timestamp > _timestamp) {
    if (_registering) _finalize_registration();
    if (_dumping) fprintf(_ofile, "#%d\n", timestamp);
  ...
  if (it != _vars_prevs.end()) {
    if (it->second == change_value) return false;
    it->second = change_value;
  } else {
    if (!reg)
      throw VCDTypeException{
          format("VCDVariable '%s' do not registered", var->_name.c_str())};
    else
...
// -----------------------------
bool VCDWriter::change(const std::string &scope, const std::string &name,
                       TimeStamp timestamp, const VarValue &value) {
...
VarPtr VCDWriter::var(const std::string &scope, const std::string &name) const {
  ...
  //     throw VCDPhaseException{ format("Such scope '%s' does not exist",
  //     scope.c_str()) };
  // VarPtr pvar = std::make_shared<VCDScalarVariable>(name,
  // VCDWriter::var_def_type, 0, *it_scope, 0); auto it_var = _vars.find(pvar);
  _search->vcd_scope.name = scope;
  _search->vcd_var._name = name;
  auto it_var = _vars.find(_search->ptr_var);
  if (it_var == _vars.end())
    throw VCDPhaseException{format("The var '%s' in scope '%s' does not exist",
                                   name.c_str(), scope.c_str())};
  return *it_var;
}
...
void VCDWriter::set_scope_type(std::string &scope, ScopeType scope_type) {
  ...
  if (it == _scopes.end())
    throw VCDPhaseException{
        utils::format("Such scope '%s' does not exist", scope.c_str())};
  (**it).type = scope_type;
}
...
// -----------------------------
void VCDWriter::_scope_declaration(const std::string &scope, size_t sub_beg,
                                   size_t sub_end) {
  const std::string SCOPE_TYPES[] = {"begin", "fork", "function", "module",
                                     "task"};
...
void VCDWriter::_write_header() {
  for (uint64_t i = 0; i < VCDHeader::kws; ++i) {
...
void VCDWriter::_finalize_registration() {
  ...
  if (_vars_prevs.size()) {
    ...
    if (!_dumping) _dump_off(_timestamp);
  }
  _registering = false;
}

// -----------------------------
VCDVariable::VCDVariable(const std::string &name, VariableType type,
                         unsigned size, ScopePtr scope, unsigned next_var_id)
...
std::string VCDVariable::declartion() const {
  return format("$var %s %d %s %s $end", VAR_TYPES[int(_type)].c_str(), _size,
                _ident.c_str(), _name.c_str());
}
...
VarValue VCDVectorVariable::change_record(const VarValue &value) const {
  if (value.size() > _size)
    throw VCDTypeException{format("Invalid binary vector value '%s' size '%d'",
                                  value.c_str(), _size)};
  ...
  for (auto i = 1u; i < (val_sz - 1); ++i) {
    char &c = val[i];
    c = tolower(val[i]);
    if (c != VCDValues::ONE && c != VCDValues::ZERO && c != VCDValues::UNDEF &&
        c != VCDValues::HIGHV)
      throw VCDTypeException{format(
          "Invalid binary vector value '%s' size '%d'", val.c_str(), _size)};
  }
  ...
  // align
    ...
    for (auto i = (val_sz - 1); i >= 1; --i) val[k + i] = val[i];
    for (auto i = 1u; i <= k; ++i) val[i] = VCDValues::ZERO;
  }
  return val;
}
