...
namespace latch {
    class Runtime {
    public:
        static void Delay(uint64_t duration); // 静态成员函数声明
    };
}
...
sc_simcontext::crunch( bool once )
{
	...
	while( true )
	{
        ...
        if(isSyncWithLatch){
            uint64_t delay_time = m_delta_time.value() / 100;
            if(delay_time > 0){
                latch::Runtime::Delay(delay_time);
                m_delta_time = SC_ZERO_TIME;
            }
        }
