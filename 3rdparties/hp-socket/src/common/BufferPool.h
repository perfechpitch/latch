...
public:
	int PushTail(const BYTE* pData, int len)
	{
		int cat = __super::PushTail(pData, len);
		this->length += cat;
	...
	{
		int fetch	  = __super::Fetch(pData, len);
		this->length -= fetch;

		return fetch;
	}

	int Increase(int len)
	{
		int increase  = __super::Increase(len);
	...
	int IncreaseLength	(int len) {return (this->length += len);}
	int ReduceLength	(int len) {return (this->length -= len);}
