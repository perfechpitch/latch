 ...
 * 带 task_last：一个 task 只拆成这一笔搬运，做完就该通知 TS */
static void dte_move(u32 src, u32 dst, u32 len, u32 mode) {
...
TASK void task_dte_move(void) {
  dte_move(CMEM_TOKEN_OFF, 0, TOKEN_BYTES, DTE_MODE_CMEM_TO_ROUTER);
  task_yield();
}
...
TASK void task_dte_concat(void) {
  dte_move(0, CMEM_TOKEN_OFF, TOKEN_BYTES, 0);
  task_yield();
}

/* 与外部节点收发 */
TASK void task_dte_fifo_in(void) {
  dte_move(0, 0, TOKEN_BYTES, 0);
  task_yield();
}

TASK void task_dte_fifo_out(void) {
  dte_move(0, 0, TOKEN_BYTES, 2);
  task_yield();
}
...
/* 把一层 MoE 那一段算完的结果发给下游。段首那 16 B 是软件辅助信息，一起发。
 * 落到 R core 的哪个槽、哪一半，包头里带着走：链上每个 core 算出来的是同一个
 * 值，Router 归约时照抄首份分量的包头，末端那一份带的就是它 */
TASK void task_dte_send_moe(void) {
  dte_move(MOE_OUT_OFF, rc_land(user_id(), smem_read(MOE_SEND_HALF_OFF)),
           MOE_SEND_BYTES, DTE_MODE_CMEM_TO_ROUTER);
  task_yield();
}
...
/* 链二的第二步：把那个槽的两笔从 Matrix Mem 搬到 Core Mem。两边都连着摆，
 * 一笔搬运就够 */
TASK void task_dte_rc_load(void) {
  ...
  dte_move(RC_MM_BASE + slot * RC_SLOT_BYTES, RC_A_OFF, RC_SLOT_BYTES,
           DTE_MODE_MMEM_TO_CMEM);
  task_yield();
}

/* 链二的最后一步：求和结果送下一组的 R core。落到那边哪个槽的哪一半，按同一
 * 条规则算 —— 上游组的中间结果落后一半 */
TASK void task_dte_rc_send(void) {
  dte_move(RC_SUM_OFF, rc_land(user_id(), 1), MOE_SEND_BYTES,
           DTE_MODE_CMEM_TO_ROUTER);
  task_yield();
}
...
TASK void task_dte_bc_send(void) {
  ...
  dte_move(bc_land(slot), 0, BC_TOKEN_BYTES, DTE_MODE_MMEM_TO_ROUTER);
  task_yield();
}
...
TASK void task_dte_bc_relay(void) {
  ...
  dte_move(bc_land(slot), bc_land(n), BC_TOKEN_BYTES, DTE_MODE_MMEM_TO_ROUTER);
  task_yield();
}
