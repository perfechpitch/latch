...
| 行 | 画什么 |
| - | - |
| `TS` | 一笔 task 从**下发**到它做完 DSA 的全程，同一单元的下一笔先下发了就截在那一拍。三条通道叠在这一行上，按单元用三种颜色区分 |
...
- **TS 那一行的配对**：DSA 的起手与完成边沿都带 task 与 user，按身份认回下发的那一笔：
  ...
  没有同一身份的下发的边沿，折成段标 `?`。
- **TS 那一行的截断**：索引要求一条通道里的段互不相交。同一单元的下一笔先下发了，前
  一笔截在那一拍，没画进来的那一截在 DSA 那一行看得到。`moe_lpu` 里 dot core 的 7 项
  concat 搬入就是这样：先下发的那一笔，完成落在后一笔下发之后。
