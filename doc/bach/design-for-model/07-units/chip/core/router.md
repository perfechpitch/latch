...
给实现 Router 的人：八个逐拍推进的模块各自做哪些事、端口与存储怎么定。每个 Core 一份，坏 core 与不派角色的 core 也有。
...
* 《软件栈》：“Router 软件视角”“使用示例：中间列 chip 的坏 core2、core7”“阻塞重传四步”
...
两类 credit 互不复用，管的东西、粒度、维护方、扣还时机都不同：

| | VC credit | stream credit（CoreMem credit） |
| - | - | - |
| 管什么 | 下游 VC Buffer 有没有空槽 | 目标 core 有没有空间容纳这个用户的数据 |
| 粒度 | 按下游方向加 VC，flit | 按 UserID 加方向，一个表项 |
| 谁维护 | RouterStation 的独立计数器，硬件自动维护；每个 VC 的深度由软件经 ctrl_noc 静态配置，取够覆盖 credit 往返延迟即可，不必装得下整包 | Router 是唯一有效状态；DTE 持一份 cache；TS 内另有一份本级表，按与 Router 完全一致的逻辑分配空项 |
| 何时扣 | flit 发出时扣该方向该 VC 一个；经 CoreMem 重注入的包，Output Port 识别到重注入标记才扣 | 新 UserID 的包在本级占一个表项；出核的包在 TS 下发任务前由 Router 授权 |
| 何时还 | flit 离开下游 VC Buffer 就归还 | 用户在下游 core 跑完任务链、用完 Core Mem 后发携带 UserID 的 release |
| 快慢 | 快，flit 一进一出就还 | 慢，要等那个用户在下游 core 上跑完整条任务链 |

一项 stream credit 同时代表目标 core 的 Core Mem 与 ReduceMemory 容量：两块存储按同样的办法给 16 个用户等分，一个用户拿到一项就两边都有位置，所以逐级归约往下游发不另记一笔。
...
<text x="20" y="26" font-size="12" fill="#111827">Router · 第 0 层（八个逐拍推进的模块；每 Core 一份，坏 core 也有。方位照 MAS 框图：core 侧模块在上，left / right 在两侧，mid 朝另一排在下）</text>
<text x="884" y="26" font-size="9.5" fill="#6b7280">灰线 = flit 数据面　橙线 = 三类 credit 与 release　绿线 = 与 TS 的控制通路　紫虚线 = ctrl_noc 配置</text>
<rect x="40" y="120" width="330" height="208.5" rx="4" fill="#f8fafc" stroke="#374151"/>
...
<text x="52.0" y="171.5" font-size="8.5" fill="#475569">字段：cur_vc · flow_dir · nxt_vc · streamNeedMask</text>
...
<text x="52.0" y="293.0" font-size="8.5" fill="#475569">core_bad_mask：10 bit，本 core 那一位为 1 就进透传档</text>
<text x="358" y="319.5" font-size="8.5" fill="#9ca3af" text-anchor="end">软件经 R2CU 接口配置</text>
...
<text x="412.0" y="266.0" font-size="8.5" fill="#475569">坏 core 不接收溢流，coremem credit 直接 bypass</text>
...
<text x="772.0" y="185.0" font-size="8.5" fill="#475569">三态准入在 Xbar：UserID 已分配 → 直接发；未分配但有空项 → 记录占用；</text>
<text x="772.0" y="198.5" font-size="8.5" fill="#475569">　无空项 → 不往 Core 出口发，那一笔留在它那个 VC 上等</text>
...
<text x="1212.0" y="252.5" font-size="8.5" fill="#475569">ReduceModule 的动作：本地分区当场放；下游映射</text>
<text x="1212.0" y="266.0" font-size="8.5" fill="#475569">　等在途任务的 Reduce release 都回来才清</text>
...
<text x="182.0" y="541.0" font-size="8.5" fill="#475569">坏 core（透传档）：不投递本 core、不占 stream 坑，VC credit 照查</text>
...
<text x="1312.0" y="527.5" font-size="8.5" fill="#475569">结果流水：元素加完就发，整包锁定到尾 flit</text>
<text x="1312.0" y="541.0" font-size="8.5" fill="#475569">　发送前查目标 VC credit</text>
<text x="1312.0" y="554.5" font-size="8.5" fill="#475569">Downstream Reduce Credit Map：按 UserID 加方向，每笔任务准入一次、</text>
...
<text x="1772.0" y="541.0" font-size="8.5" fill="#475569">坏 core（透传档）：不投递本 core、不占 stream 坑，VC credit 照查</text>
...
<text x="802.0" y="880.0" font-size="8.5" fill="#475569">坏 core（透传档）：不投递本 core、不占 stream 坑，VC credit 照查</text>
...
<path d="M271.0 328.5 L272.0 394.0" stroke="#475569" stroke-width="1.3" fill="none" stroke-linejoin="round" stroke-dasharray="4 3" marker-end="url(#a)"/>
...
<text x="20" y="1134" font-size="10.5" fill="#374151" text-anchor="start">本级 Rmem 的一份由 TS 按用户记，下发一笔 reduce 占掉、Rmem 做完才还；下游那边的容量随 stream credit 一起给，Router 不另记。ReduceModule 输入三路各 160 GB/s，输出 160 GB/s。</text>
</svg>
```
...
| F14 | Credit Release 静态旁路：release 不查 RouterTable、不做动态路径选择、不进 Xbar 仲裁，只按 CSR 配的静态方向 Mask 转发；Mask 含多个方向时同一笔 release 复制到所有指定方向，UserID 保持不变。收到的那一笔一律先交本级，所以只有要把 release 送过去的口上配得有 Mask |
| F15 | 坏 core 上的 RouterStation 处在透传档（F57）：数据走完整流水线，但只按 RouterTable 往 `mid`、`left`、`right` 转发，不投递本 core、不进 ReduceModule，不占 stream 坑、不扣 Core Mem 的量、不做溢流转存；下游还回来的 release 按 `RTR_RELEASE_ROUTE` 静态转发。链路级的 VC credit 照查照记：它记的是下一跳 VC Buffer 还剩几个位置，与本 core 坏不坏无关，不查就会把下一跳撑爆。不派角色的好 core 不开透传档，只转发由它的表项决定 |
...
| F23 | 交织粒度分两种：Router 到 Router 的通路允许在 flit 边界换到另一个 VC 的包，需保存 VC、输出方向、剩余长度和包边界上下文，同一出口同一 VC 上一个包从首 flit 占到尾 flit，中间断拍时别的包也不能插进这个 VC；包一旦开始进入 Core 或 ReduceModule 就锁定到尾 flit。两种粒度不能混用 |

### CoreStation

| 编号 | 功能 |
| - | - |
| F24 | 进 core 的三态准入在 Xbar 那一级做：UserID 已分配则直接发；未分配但进本 core 那张 stream 表有空项则记录占用后发；无空项时不往 Core 出口发，那一笔留在它那个 VC 上等，VC 有空项时仍可继续接收上游数据。到了 CoreStation 的包一定收得下，收不下是电平没拉住，断言 |
...
| F28 | 按接收包头的顺序经 `router2ts_trigger_ch` 直接通知 TS，**Header 就绪即通知，不等整包收完**，请求里带 `user_id`、`path_id` 与这一笔要不要重发的标记。这条通路硬件直连，不经 RV core，也不经任何软件环节 |
...
| F38 | 同一 User 同一包的第一份输入写进这个用户的分区（FP32），后续方向的输入读出当前值、累加、写回，原位 Read-Modify-Write |
...
| F42 | 结果按 flit 出：某个结果 flit 所需的各方向操作数都累加完了，这个 flit 就能发，不等整包；一个包发出首 flit 后锁定到尾 flit，后面的结果没好就占着输出等。发送前查目标 VC credit，作为 Xbar 的第五路输入参与仲裁 |
...
| F44 | “全部方向”取自 RouterTable 的 `reduce_in_mask`：按包头的 `path_id` 查 `rdc_rtab[path_id].reduce_in_mask`，得到这条 path 在本级会有哪几个相邻方向送来分量。某个结果元素要等集合里各方向对应的那个元素都进来。一笔任务的首份输入进来时把这个集合一并记进 `rdc_user_tab`，这笔任务做完前不再重查 |
| F45 | `reduce_in_mask` 为 0 表示本级不做累加：坏 core 与这条 path 上只转发的好 core 都是这一档，包按 `flow_dir` 直接转发，不进 ReduceModule |
| F46 | 结果往下游发不查下游的 ReduceMemory：那边的容量与它的 Core Mem 一起分配，上游发数据进那个 core 时取得的那一项 stream credit 同时代表两者，逐 flit 只受 VC credit 约束 |
| F47 | Downstream Reduce Credit Map 按 UserID 加目标方向记相邻下游能不能接这个用户的下一笔任务：取得准入时置忙，收到 release 时放开 |
| F48 | 整包发出后经 `rmem2ts_done_ch` 向 core 返回 UserID、PathID 与该包包头里的 `reduce_seq`。TS 只认这一路把逐级 reduce 任务置 FINISH：按 UserID 找到那个用户，完成的是它的当前任务 |
| F49 | 输入侧只有 VC credit 准入，`credit > 0` 即收 |
| F50 | 表项的建与删：收到本级 core 该 UserID 的 Retire 后本地分区可以回收；相邻下游各方向在途任务的 release 全部回来之后，才删掉这个用户的映射给其他用户用 |
...
| 编号 | 功能 |
| - | - |
| F53 | 64 条表项，按 `path_id` 索引。字段照 DATA_NOC HAS 的 `Routing table field`，VC 与阻塞那几项照 Router MAS 的 `Table Entry`：<br>`op_type` 2 bit（0 = kernel / weight 搬运、1 = transfer、2 = reduce、3 = reduce_twice）<br>`flow_dir` 5 bit 出方向掩码（bit0 上下、bit1 左、bit2 右、bit3 reduce1、bit4 reduce2）<br>`cur_vc` 2 bit、`nxt_vc` 五个出方向各 2 bit<br>`path_core_mask_enable` 1 bit、`path_core_mask_idx` 4 bit、`path_core_bypass` 1 bit（0 进 core、1 bypass）<br>`need_buffer` 1 bit（这条 path 允许进 core 缓存，即溢流使能）<br>`streamNeedMask` 4 bit（哪几个出口发之前要查那一侧的 stream credit table：bit0 上下、bit1 左、bit2 右、bit3 进本 core）<br>`cur_credit_type` 1 bit（0 广播 / 1 P2P）、`cur_credit_require` 6 bit<br>`nxt_credit_type` 3 bit（三个 R2R 方向各一位）、`nxt_credit_require` 三个 R2R 方向各 6 bit<br>`reduce_data_type` 3 bit（输入精度 BF16 / FP32）、`reduce_outdata_type` 1 bit（输出精度）<br>`reduce_in_mask` 3 bit、`operation` 2 bit（0 转发、1 reduce0 单流，经 Rmem 不计算、2 reduce1 两流、3 reduce2 三流）、`stall_way` 1 bit |
...
| F57 | 另有一个与 RouterTable 分开配的 **`core_bad_mask` 寄存器**：每颗 chip 10 bit，第 i 位为 1 表示 core i 是坏 core，每个 core 都存这 10 位。每颗 chip 至多 2 个坏 core，一行至多 1 个。Router 只看本 core 那一位，这一位为 1 就进透传档（F15、F70）。真机上它是上电时从 eFuse 读出并锁存的；模型里在 Router 配置阶段先于 RouterTable 与 `RTR_RELEASE_ROUTE` 写入，业务开始之后不再变。复位释放后 RouterTable 的所有条目为 bypass / no-op，配置写入前不投递任何包 |
...
| F63 | Credit Bypass Route（寄存器 `RTR_RELEASE_ROUTE`）：软件经 CSR 为每个业务 credit 输入端口配置静态输出方向 Mask，可指定一个或多个 R2R 方向；隔在两个好 core 之间的坏 core 靠它把 release 送到上游，好 core 一律交给本级，不配 Mask |
| F64 | 中间节点可以按表改写 VC |
...
| F67 | 暂存空间由软件在 Core Mem 里预留，容量与项数取自 `cmem_part` 的 `reissue_base` 与 `reissue_pkts_per_vc`，每 VC 至少容得下一个整包。`pending_reinject` 计数到这个上限说明配少了，**模型直接断言失败**：不覆盖已暂存的包，不丢包，也不退回“留在当前 VC 等”，因为那条路会让同一个 `stall_way` 配置在两种容量下走出两种行为，掩盖配置错误 |
...
| F70 | 坏 core 不接收溢流：Router 对它不发起进 core 缓存处理，`stall_way` 只能是留在 VC 等待，业务 credit pulse 无效，coremem credit 直接 bypass |
...
port link[d] (双向, credit/release, clk)          // d ∈ {left, right, mid}：256 B/T，接相邻 core 的 Router 或 C2C Bridge
  ...
  in  stream_release_vld · stream_release_user[15:0]      // stream credit release，与数据反向走
  out 同字段
...
port rmem2ts_done_ch (master, 脉冲, clk)              // ReduceModule → TS：一个整包 reduce 完成
  out valid · user_id[15:0] · path_id[7:0] · reduce_seq[5:0]   // reduce_seq 原样取自包头
...
```
mem rtab[k]           FF 阵列   64 × {op_type[1:0], flow_dir[4:0], cur_vc[1:0], nxt_vc[4:0][1:0], path_core_mask_enable, path_core_mask_idx[3:0], path_core_bypass, need_buffer, stream_need_mask[3:0], cur_credit_type, cur_credit_require[5:0], nxt_credit_type[2:0], nxt_credit_require[2:0][5:0], reduce_data_type[2:0], reduce_outdata_type, reduce_in_mask[2:0], operation[1:0], stall_way, ext_dst[5:0]}  1R1W  多副本，逐份写  复位 全条目 bypass / no-op
mem core_bad_mask     FF        10 b，第 i 位对应 core i                                     1R1W  Router 配置阶段先于 RouterTable 写入，业务开始后不变  复位 0
mem rtab_commit       FF        {副本写入游标, 完成状态}                                    1RW   更新状态机                复位 空闲
mem credit_bypass     FF 阵列   每个业务 credit 输入端口一个 {out_mask[3:0]}：mid、left、right、本 core                1R1W  CSR 配置                  复位 0
...
mem rdc_down_credit   FF 阵列   16 用户 × 3 方向 × 忙标志                                    1RW   任务头一次提交时置忙，release 放开  复位 空闲
mem rdc_out_q         队列      每个在做的任务一项，最多 16 项                               1W1R  队头发出首 flit 后锁定到尾 flit  复位空
...
`rtab`（64 项按 `path_id` 索引）、`core_bad_mask`（每颗 chip 一个 10 bit 的值，写进这颗 chip 的全部 10 个 core：第一列与最后一列 chip 是 `0x000`，中间两列 chip 是 `0x084`，即 core2、core7 是坏 core）、`credit_bypass`（每个业务 credit 输入端口一个 `out_mask`）都由编译侧算好，boot 期经 ctrl_noc 写入。同一个 `path_id` 在不同 core 上表项不同，`rtab` 因此**每 core 一份**，不能从拓扑反推。`CreditCounter[path_id][stream_id]` 的初值同样每 core 一份：广播任务等于目的 core 数量，P2P 任务是 1。
...
| Core | Path | flow_dir | path_core_bypass | streamNeedMask | operation | stall_way | reduce_in_mask |
| - | - | - | - | - | - | - | - |
| C8 | chip 内归约 | 左 | 1 不进核 | 0 | Reduce0 | 留在 VC | bit0 |
| C7 坏 core | chip 内归约 | 左 | 1 不进核 | 0 | 转发 | 只能留在 VC | 0 |
| C6 | chip 内归约 | 左 | 1 不进核 | 0 | Reduce1 | 留在 VC | bit0、bit2 |
| C5 | chip 内归约 | 上下 | 1 不进核 | 0 | Reduce1 | 留在 VC | bit0、bit2 |
| C0 | chip 内归约 | 右，加 reduce1 | 1 不进核 | 0 | Reduce1 | 留在 VC | bit0、bit1 |
| C1 | chip 内归约 | 右 | 1 不进核 | 0 | Reduce1 | 留在 VC | bit0、bit1 |
| C2 坏 core | chip 内归约 | 右 | 1 不进核 | 0 | 转发 | 只能留在 VC | 0 |
...
按数据流分，concat 每个上游 core 一条，其余每种数据流一条：

| 数据流 | 走法 |
| - | - |
| B core 进口 | 入口桩 → 第一个 B core 的 datain |
| token 广播 | B core 广播给本组 64 个计算 core |
| 组间转发 | B core 转给下一组的 B core |
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 945 198" font-family="PingFang SC, Noto Sans CJK SC, Microsoft YaHei, sans-serif">
  ...
  <text x="250" y="118" font-size="10.5" fill="#475569">3. stream_need = e.streamNeedMask；enter_core = e.path_core_mask_enable ? core_mask[e.path_core_mask_idx] : !e.path_core_bypass</text>
  ...
  <text x="250" y="162" font-size="10" fill="#9ca3af">坏 core 只置转发位，need_stream 与 enter_core 恒为 0</text>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 950 326" font-family="PingFang SC, Noto Sans CJK SC, Microsoft YaHei, sans-serif">
  ...
  <text x="842" y="197" font-size="9.5" fill="#6b7280" text-anchor="middle">stream_release_vld</text>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 903 198" font-family="PingFang SC, Noto Sans CJK SC, Microsoft YaHei, sans-serif">
  ...
  <text x="795" y="133" font-size="9.5" fill="#6b7280" text-anchor="middle">ready</text>
  ...
  <text x="250" y="98" font-size="10.5" fill="#475569">2. done → trigger = {user_id, path_id, reissue}</text>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 850 218" font-family="PingFang SC, Noto Sans CJK SC, Microsoft YaHei, sans-serif">
  ...
  <text x="742" y="87" font-size="10" fill="#334155" text-anchor="middle">ready_elem</text>
  ...
  <text x="250" y="158" font-size="10.5" fill="#475569">4. ready_elem = min(各路已进来的元素数)，收齐的那一路不算</text>
  <text x="250" y="182" font-size="10" fill="#9ca3af">同一 User 的下一笔在结果全部发出前不得覆盖该分区</text>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 950 198" font-family="PingFang SC, Noto Sans CJK SC, Microsoft YaHei, sans-serif">
  ...
  <text x="104" y="77" font-size="10" fill="#334155" text-anchor="middle">ready_elem</text>
  ...
  <text x="842" y="72" font-size="10" fill="#374151" text-anchor="middle">rdc_out_q · 在做的任务 ≤ 16 · 1W</text>
  ...
  <text x="250" y="118" font-size="10.5" fill="#475569">3. 每个 flit：vc_credit[o][nxt_vc] &gt; 0 且本 flit 的元素都 &lt; ready_elem</text>
  <text x="250" y="138" font-size="10.5" fill="#475569">4. 尾 flit 发完 → rmem2ts_done_ch.valid = 1，给上游还 release</text>
  <text x="250" y="162" font-size="10" fill="#9ca3af">发出首 flit 后锁定到尾 flit，后面的结果没好就占着输出等</text>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 884 198" font-family="PingFang SC, Noto Sans CJK SC, Microsoft YaHei, sans-serif">
  ...
  <text x="250" y="56" font-size="12" fill="#111827">Retire · 立即删表项，本地分区当场放</text>
  ...
  <text x="250" y="118" font-size="10.5" fill="#475569">3. ReduceModule：该 user 没有在做的任务 → 当场放分区</text>
  <text x="250" y="138" font-size="10.5" fill="#475569">4. rdc_down_credit[user][*] 等在途任务的 release 回来才清</text>
...
单跳延迟            ≤ 6 cycles（六级流水线），优化后 4～5；过坏 core 透传档的那一跳延迟与正常跳一致
RouterTable         64 条表项；字段含 flow_dir（出）与 reduce_in_mask（进）两个方向集合；内部副本数与每副本写入拍数 5、1（待定）
core_bad_mask       每颗 chip 10 bit，第 i 位对应 core i；每颗 chip 至多 2 个坏 core，一行至多 1 个
...
operation 的 reduce0 / reduce1 / reduce2     单流经 Rmem 不计算 / 两流 / 三流（Router MAS）
...
| `core_bad_mask` 与 RouterTable 分开配、先于它写入，本 core 那一位打开透传档；复位后全条目 bypass / no-op | F57 | `core_bad_mask_config` |
...
| 进 core 后锁定到尾 flit，R2R 可在 flit 边界换 VC，同一 VC 上包不交织 | F23 | `interleave_grain` |
...
| 分区在用户第一笔任务进来时分配，用光了对输入反压，Retire 才放 | F51 | `reduce_ctx_on_demand` |
| 结果按 flit 出，整包锁定到尾 flit；每 flit 查 VC credit | F42、F47 | `reduce_out_credit` |
...
| Retire 的三方时序：Core 保证 → Router 立即删表项 → ReduceModule 当场放分区、下游映射等 release | F72～F75 | `retire_three_party` |
...
| A15 | 单个坏 core | 它在透传档只转发不记账，stream 表全空、不发 trigger、不参与重发；上游记的是它下游那个 core 的坑，下游认的上游是它上游那个 core；释放经它透传回上游 |
| A16 | 一串只转发的 core，其中含坏 core，例如行链在中间列 chip 上经过的 core5、core6、core7、core8 | 逐跳链式转发，两端仍互认为直接上下游，释放沿这串 core 逐跳回传，转发时延是逐跳 R2R 累加而不是单跳 |
...
* **为什么分区在用户第一笔任务进来时才分配**
  * 只有真做 reduce 的用户才占分区，只收发普通数据的用户不占
  * 代价是 16 个分区用光时要对新用户的输入反压，等别的用户 Retire
* **为什么结果按 flit 出，不等整包**
  * 逐级 reduce 的每一跳不必等整包收齐，各路首 flit 到了结果首 flit 就能往下一跳走，每跳省掉一整包的收包时间
  * 代价是包发出首 flit 后要占着出口等后面的结果：Xbar 上同一出口同一 VC 被这个包独占到尾 flit
...
* **为什么透传档只跟 `core_bad_mask` 走**
  * 《DATA_NOC DE HAS》把“坏 core 不接收溢流、业务 credit pulse 无效”绑在 `core_bad_mask` 上，坏 core 上只有 Router 能用
