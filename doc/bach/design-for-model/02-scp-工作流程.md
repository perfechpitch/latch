...
* ④ 解复位：SCP 写 clk / reset 模块
  ...
  * WFT 指令：通知 TS 允许下发新任务；可选通知 IPI 向 SCP 上报状态（boot 阶段用）。它不是那 6 条自定义指令之一，编码取 custom-0 的空档 `funct3 = 011`、操作数字段全 0（`0x0000300B`），`bit31` 留给 IPI 那一位。固件侧当前写在 `src/bach/compiler/kernel/start.s` 的收尾，编码是 F14 那条不通知 TS 的 `task_done`（`funct3 = 010`、`0x0000200B`），不是 WFT。模型不跑 firmware 那一段，也不实现 WFT 011；TS 下发任务时直接跳到 `task_pc`
