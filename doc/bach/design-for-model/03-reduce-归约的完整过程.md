...
<text x="905.0" y="80" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="11.5" fill="#7c3aed" font-weight="700" text-anchor="middle">第三层　EP 组间：沿 R core 链逐行累加</text>
...
<text x="58" y="145" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#5c6370" font-weight="400" text-anchor="start">对本 core 上每个激活的专家：FC2 输入 × 该专家的 FC2 分片，</text>
...
<text x="438" y="126" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#16181d" font-weight="700" text-anchor="middle">core 6</text>
...
<text x="516" y="126" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#16181d" font-weight="700" text-anchor="middle">core 5</text>
...
<text x="594" y="126" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#16181d" font-weight="700" text-anchor="middle">core 0</text>
...
<text x="672" y="126" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#16181d" font-weight="700" text-anchor="middle">core 1</text>
...
<text x="550" y="220" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#5c6370" font-weight="400" text-anchor="middle">p0 → core5 的 ReduceModule 与 p1 原位相加 → core0 …</text>
...
<text x="550" y="328" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#2563eb" font-weight="600" text-anchor="middle">数据在哪等：ReduceModule 上下文 16 × 32 KiB</text>
<text x="550" y="344" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#2563eb" font-weight="600" text-anchor="middle">流控：本级由 TS 按用户记，下游 Reduce credit 由 ReduceModule 管</text>
...
<text x="830" y="124" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#5c6370" font-weight="400" text-anchor="middle">行 0：4 颗 chip 的行链</text>
...
<text x="830" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#5c6370" font-weight="400" text-anchor="middle">行 1：4 颗 chip 的行链</text>
...
<text x="830" y="212" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#5c6370" font-weight="400" text-anchor="middle">行 2：4 颗 chip 的行链</text>
...
<text x="830" y="256" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#5c6370" font-weight="400" text-anchor="middle">行 3：4 颗 chip 的行链</text>
...
<text x="905" y="292" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#5c6370" font-weight="400" text-anchor="middle">…… 到第 11 行的 R core 才出核</text>
...
<text x="46" y="443" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">第二层与第三层不共用机制：EP 组间若也走 Router 逐跳累加，ReduceModule 的 16 × 32 KiB 上下文盖不住 EP 之间的不均衡，任务少的组会被频繁反压，所以第三层借一个 core 的 32 MB Matrix Mem 做缓冲。</text>
...
* `A` 是 FC2 输入，`B` 是该专家的 FC2 分片，精度取 BF16、MXFP8 或 MXFP4，默认用例都是 MXFP8
* `W_ep` 是这个专家的权重，FP32
* 不支持初始 `C` 加载：第一个专家的结果直接写 `C`，后续专家逐个累加
* 输出 FP32 或 BF16，默认用例 BF16

这条路径把 EP reduce 从 VU 挪进 MU，Core Mem 不必为每个用户缓存所有激活专家的中间结果。建模时 MoE 多专家场景默认走它。VU 保留跨元素归约原语（VALU1 的求和 / 最大 / 最小），是备用路径。

在任务链里，第一层要么是 FC2 的那个 MU task 本身，要么是紧跟其后的一个 VU task（《软件栈》里 normal core 的示例链写作 task 5 “Core 内 reduce”）。第一层的输出是本 core 对这个 token 的一份完整长度或一段的结果。完整长度的直接交给第二层；一段的先在 chip 内 concat 成完整向量再交给第二层，默认用例的 FC2 就是这种，每个 core 出 6144 维里的一段。
...
默认用例是 `tp_nk`，EP6 + TP8。第二层在一颗 chip 内与一行 chip 之间各出现一次，两次都经过 dot core。dot core 指每颗 chip 的逻辑 core7：编译器把每颗 chip 的 8 个计算 core 当 2×4 格子用，槽位号 = 行 × 4 + 逻辑列，槽位 7 那个 core 就是 dot core；它在第一列与中间两列 chip 是物理 core9，在最后一列 chip 是物理 core8。
...
中间两列 chip 的 core2、core7 是坏 core：只有 Router 能用，Router 处在透传档，数据只按 RouterTable 转发，不投递本 core，不进 ReduceModule。两条链经过坏 core 时只透传。

chip 内归约链一律按逻辑槽位次序 6 → 5 → 4 → 0 → 1 → 2 → 3 → 7 走，物理上：

| chip 所在列 | 物理次序（括号是透传的坏 core） |
| - | - |
| 第一列 | core8 → core7 → core6 → core1 → core2 → core3 → core4 → core9 |
| 中间两列 | core8 →（core7）→ core6 → core5 → core0 → core1 →（core2）→ core3 → core4 → core9 |
| 最后一列 | core7 → core6 → core5 → core0 → core1 → core2 → core3 → core8 |
...
<text x="30" y="46" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10" fill="#5c6370" font-weight="400" text-anchor="start">以中间列 chip 的 chip 内归约链上相邻的 core6 → core5 → core0 → core1 为例。每个 core 的 DTE 把部分和当 Reduce 包发出，Router 按 operation 导进 ReduceModule，与已到的上下文相加后送下一跳</text>
<rect x="40" y="70" width="240" height="300" rx="8" fill="#fbfcfd" stroke="#9aa1ad" stroke-width="1.2"/>
<text x="160" y="88" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="12" fill="#16181d" font-weight="700" text-anchor="middle">core 6</text>
...
<text x="450" y="88" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="12" fill="#16181d" font-weight="700" text-anchor="middle">core 5</text>
...
<text x="740" y="88" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="12" fill="#16181d" font-weight="700" text-anchor="middle">core 0</text>
...
<text x="1030" y="88" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="12" fill="#16181d" font-weight="700" text-anchor="middle">core 1</text>
...
6. 这笔任务的结果全部交付后，向上游发一次携带 UserID 的 release；输入侧 flit 离开 VC Buffer 时归还 VC credit。
7. 最后一跳的去向由 RouterTable 定：进本 core 的 Core Mem，或继续送下一 chip、送 R core。默认用例里 chip 内归约链的链尾是 dot core，结果落进它的 Core Mem；行链的最后一跳落进本行 R core 的 Matrix Mem。
8. 整包发出后 ReduceModule 向 core 返回 UserID（`rmem2ts_done_ch`），TS 只认这一路把 reduce task 置 FINISH。

链上没有同步点：`p0` 到 core5 时不必等 core5 算完，先存进上下文，core5 的 `p1` 出核时再加。这是“流着加”，也是 chip 间 reduce 不做中途汇聚的理由。
...
<text x="464" y="129" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">Reduce Context SRAM：16 用户 × 32 KiB</text>
...
<text x="464" y="285" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">一笔任务准入一次，按 release 恢复</text>
...
<text x="464" y="349" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">下游在途任务的 release 都回来才删映射</text>
...
<text x="46" y="439" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">输入侧只有 VC credit 准入（credit &gt; 0 即收），Reduce credit 是另一张网，按用户、按任务记，与数据面分离。</text>
<text x="46" y="455" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">边缘的 ReduceModule 因跨 chip 带宽有限，HAS 要求它自己做精度压缩；release 一笔任务一次，跨 chip 时经 C2C Bridge 透传。</text>
</svg>
```
...
| 结果生成与发送 | 结果 flit 的操作数加完就发，不等整包，包锁定到尾 flit；输出精度可配 FP32 或 BF16；发送前检查目标 VC credit 与下游 Reduce credit |
| Downstream Reduce Credit Map | 按 UserID 加相邻下游方向记能不能接这个用户的下一笔任务：一笔任务每个方向准入一次，按 release 放开 |
...
<text x="30" y="46" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10" fill="#5c6370" font-weight="400" text-anchor="start">本级由 TS 按用户记，每个 ReduceModule 按用户、按任务持相邻下游的；Router 不持有，只把 release 按静态 Mask 转发</text>
...
<text x="48" y="119" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">本级 Rmem 资源</text>
<text x="48" y="131" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">TS 下发前申请</text>
<text x="48" y="143" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">链上逐项顺序申请</text>
...
<text x="288" y="131" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">一笔任务每方向准入一次</text>
...
<text x="548" y="143" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">做完一笔 release 一次</text>
...
<text x="505" y="140" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#2563eb" font-weight="400" text-anchor="middle">一笔任务一次</text>
...
<text x="487" y="222" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#d97706" font-weight="400" text-anchor="middle">做完一笔，release(Valid + UserID)</text>
...
<text x="228" y="258" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#d97706" font-weight="400" text-anchor="middle">做完一笔经 rmem2ts_done_ch 报 TS</text>
...
<text x="48" y="319" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">一笔 reduce 拆成链上 N 项：每项等本用户的 Rmem credit（rmem_busy = 0），</text>
<text x="48" y="331" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">到位即下发，Reduce Done 报回才还，下一项接着申请</text>
...
<text x="378" y="331" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">ReduceModule 先记录，待相邻下游各方向在途任务的 release 都回来才删该用户映射</text>
...
<text x="728" y="319" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">release 经 C2C Bridge 透传；</text>
<text x="728" y="331" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">边缘 ReduceModule 自行精度压缩</text>
<rect x="30" y="370" width="1040" height="46" rx="5" fill="#f5f6f8" stroke="#9aa1ad" stroke-width="1.2"/>
<text x="46" y="387" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">计数实体在各 ReduceModule 的 ResourceMap；本级那一段由 TS 按用户记；Router 的 CreditBypass XBar 只转发。用户的映射在 Retire 且在途任务的 release 都回来后删除。</text>
...
| - | - | - | - | - |
| core 与本级 ReduceModule 之间 | TS，每个用户一份 | TS 下发一项逐级 reduce 任务时；链上连着几项时逐项下发，一项做完再发下一项 | ReduceModule 做完一笔任务、结果全部交付后经 `rmem2ts_done_ch` 向 TS 报完成 | 用户首笔 Reduce 任务进入时分配 32 KiB 分区；收到 Retire 后回收 |
| ReduceModule 与相邻下游之间 | 上游 ReduceModule 的 Downstream Reduce Credit Map，按 UserID 加方向 | `reduceNeedMask` 置位时，一笔任务头一次发往该方向前取得准入；逐 flit 另查目标 VC credit | 下游做完这笔任务、结果全部交付后发一次携带 UserID 的 release，经 Router 的静态旁路返回 | ReduceModule 收到 User Retire 先记录，待相邻下游各方向在途任务的 release 都回来才删该用户映射 |

Router 不维护 Reduce credit，只把 release 按 CSR 配的静态方向 Mask（`RTR_RELEASE_ROUTE`）转发，不查 RouterTable，可多播。跨 chip 时 release 经 C2C Bridge 透传；边缘的 ReduceModule 自行做精度压缩以减少跨 chip 带宽。

归约链上隔着坏 core 的一跳，坏 core 同样配 `RTR_RELEASE_ROUTE`，把从下游一侧进来的 release 转往上游一侧。`RTR_RELEASE_ROUTE` 按入口方向配、不分 path；默认用例里 chip 内归约链与行链的走法，让同一个 core 的同一个入口方向上两条链要的去向一致。

### TS 与 DTE 侧的配合

任务链里的逐级 reduce 是 DTE 任务，一笔 reduce 拆成几笔时链上连着配几项。每项与普通 DTE 任务的差别：

| 字段 | 取值 | 含义 |
| - | - | - |
| `TASK_TYPE` | 4 | 逐级 reduce 任务，完成只认 Router 的 Reduce Done |
| `TASK_CREDIT_EN` | 1 | 下发前要有本级 Rmem 的 credit |

TS 的动作：

* 逐级 reduce 任务生成后停在 WAIT。本级 Rmem 的 credit 由 TS 为每个用户记一份（`rmem_busy`），不向 Router 申请：空闲就置 READY，下发即占掉，Reduce Done 报回来才还，链上的下一项接着申请
* 完成只认 Router：DTE 的 RV core 与 DSA 两路完成只算搬完，不改 stream 状态；**只有 Router 的 Reduce Done 才能把逐级 reduce 任务置 FINISH**
...
```svg
<svg viewBox="0 0 1200 1510" width="1200" height="1510" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="EP 组间的 reduction 链：落本组就累加，不落就只转发，链尾才出核">
<title>EP 组间的 reduction 链</title>
<rect width="1200" height="1510" fill="#ffffff"/>
<defs><marker id="rca" viewBox="0 0 10 8" refX="9" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M0 0 L10 4 L0 8 z" fill="#9aa1ad"/></marker><marker id="rcas" viewBox="0 0 10 8" refX="1" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M10 0 L0 4 L10 8 z" fill="#9aa1ad"/></marker><marker id="rcai" viewBox="0 0 10 8" refX="9" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M0 0 L10 4 L0 8 z" fill="#3f4451"/></marker><marker id="rcais" viewBox="0 0 10 8" refX="1" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M10 0 L0 4 L10 8 z" fill="#3f4451"/></marker><marker id="rcab" viewBox="0 0 10 8" refX="9" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M0 0 L10 4 L0 8 z" fill="#2563eb"/></marker><marker id="rcabs" viewBox="0 0 10 8" refX="1" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M10 0 L0 4 L10 8 z" fill="#2563eb"/></marker><marker id="rcar" viewBox="0 0 10 8" refX="9" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M0 0 L10 4 L0 8 z" fill="#d97706"/></marker><marker id="rcars" viewBox="0 0 10 8" refX="1" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M10 0 L0 4 L10 8 z" fill="#d97706"/></marker><marker id="rcac" viewBox="0 0 10 8" refX="9" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M0 0 L10 4 L0 8 z" fill="#0d9488"/></marker><marker id="rcacs" viewBox="0 0 10 8" refX="1" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M10 0 L0 4 L10 8 z" fill="#0d9488"/></marker><marker id="rcap" viewBox="0 0 10 8" refX="9" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M0 0 L10 4 L0 8 z" fill="#7c3aed"/></marker><marker id="rcaps" viewBox="0 0 10 8" refX="1" refY="4" markerWidth="7" markerHeight="6" orient="auto"><path d="M10 0 L0 4 L10 8 z" fill="#7c3aed"/></marker><pattern id="rcbad" width="4" height="4" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="4" height="4" fill="#e5e7eb"/><path d="M0 0 L0 4" stroke="#9aa1ad" stroke-width="1.5"/></pattern></defs>
<text x="600.0" y="26" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="15" fill="#16181d" font-weight="700" text-anchor="middle">EP 组间的 reduction 链：落本组就累加，不落就只转发，链尾才出核</text>
<text x="600.0" y="46" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="11" fill="#5c6370" font-weight="400" text-anchor="middle">按机柜的物理排布画到 core 一级；每行借最后一列 chip 的 core9 作 R core，12 个 R core 逐行串成一条链，本行 4 颗 chip 的 dot core 逐跳 reduce 进本行 R core</text>
...
<path d="M178 165 L298 165" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M178 196 L298 196" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
...
<rect x="285.0" y="187.0" width="26" height="18" rx="3" fill="#ccfbf1" stroke="#0d9488" stroke-width="1.2"/>
<text x="298" y="199.5" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#0d9488" font-weight="700" text-anchor="middle">D</text>
<rect x="342" y="140" width="176" height="78" rx="5" fill="#fdf6e3" stroke="#d97706" stroke-width="1.1"/>
<path d="M370 165 L490 165" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M370 196 L490 196" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
...
<text x="370" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">0</text>
<rect x="387.0" y="156.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="400" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">1</text>
<rect x="417.0" y="156.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="430" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">2</text>
<rect x="447.0" y="156.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="460" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">3</text>
<rect x="477.0" y="156.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="490" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">4</text>
<rect x="357.0" y="187.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="370" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">5</text>
<rect x="387.0" y="187.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="400" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">6</text>
<rect x="417.0" y="187.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="430" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">7</text>
<rect x="447.0" y="187.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="460" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">8</text>
<rect x="477.0" y="187.0" width="26" height="18" rx="3" fill="#ccfbf1" stroke="#0d9488" stroke-width="1.2"/>
<text x="490" y="199.5" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#0d9488" font-weight="700" text-anchor="middle">D</text>
<rect x="534" y="140" width="176" height="78" rx="5" fill="#fdf6e3" stroke="#d97706" stroke-width="1.1"/>
<path d="M562 165 L682 165" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M562 196 L682 196" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
...
<text x="562" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">0</text>
<rect x="579.0" y="156.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="592" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">1</text>
<rect x="609.0" y="156.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="622" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">2</text>
<rect x="639.0" y="156.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="652" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">3</text>
<rect x="669.0" y="156.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="682" y="168" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">4</text>
<rect x="549.0" y="187.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="562" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">5</text>
<rect x="579.0" y="187.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="592" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">6</text>
<rect x="609.0" y="187.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="622" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">7</text>
<rect x="639.0" y="187.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="652" y="199" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">8</text>
<rect x="669.0" y="187.0" width="26" height="18" rx="3" fill="#ccfbf1" stroke="#0d9488" stroke-width="1.2"/>
<text x="682" y="199.5" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#0d9488" font-weight="700" text-anchor="middle">D</text>
<rect x="726" y="140" width="176" height="78" rx="5" fill="#fdf6e3" stroke="#d97706" stroke-width="1.1"/>
<path d="M754 165 L874 165" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M754 196 L874 196" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
...
<rect x="831.0" y="187.0" width="26" height="18" rx="3" fill="#ccfbf1" stroke="#0d9488" stroke-width="1.2"/>
<text x="844" y="199.5" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#0d9488" font-weight="700" text-anchor="middle">D</text>
...
<path d="M178 257 L298 257" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M178 288 L298 288" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
...
<rect x="165.0" y="248.0" width="26" height="18" rx="3" fill="#f1f2f4" stroke="#9aa1ad" stroke-width="0.9" stroke-dasharray="2 2"/>
<text x="178" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#9aa1ad" font-weight="400" text-anchor="middle">0</text>
...
<rect x="285.0" y="279.0" width="26" height="18" rx="3" fill="#ccfbf1" stroke="#0d9488" stroke-width="1.2"/>
<text x="298" y="291.5" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#0d9488" font-weight="700" text-anchor="middle">D</text>
<rect x="342" y="232" width="176" height="78" rx="5" fill="#fdf6e3" stroke="#d97706" stroke-width="1.1"/>
<path d="M370 257 L490 257" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M370 288 L490 288" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
...
<text x="370" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">0</text>
<rect x="387.0" y="248.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="400" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">1</text>
<rect x="417.0" y="248.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="430" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">2</text>
<rect x="447.0" y="248.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="460" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">3</text>
<rect x="477.0" y="248.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="490" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">4</text>
<rect x="357.0" y="279.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="370" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">5</text>
<rect x="387.0" y="279.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="400" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">6</text>
<rect x="417.0" y="279.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="430" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">7</text>
<rect x="447.0" y="279.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="460" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">8</text>
<rect x="477.0" y="279.0" width="26" height="18" rx="3" fill="#ccfbf1" stroke="#0d9488" stroke-width="1.2"/>
<text x="490" y="291.5" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#0d9488" font-weight="700" text-anchor="middle">D</text>
<rect x="534" y="232" width="176" height="78" rx="5" fill="#fdf6e3" stroke="#d97706" stroke-width="1.1"/>
<path d="M562 257 L682 257" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M562 288 L682 288" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
...
<text x="562" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">0</text>
<rect x="579.0" y="248.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="592" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">1</text>
<rect x="609.0" y="248.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="622" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">2</text>
<rect x="639.0" y="248.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="652" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">3</text>
<rect x="669.0" y="248.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="682" y="260" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">4</text>
<rect x="549.0" y="279.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="562" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">5</text>
<rect x="579.0" y="279.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="592" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">6</text>
<rect x="609.0" y="279.0" width="26" height="18" rx="3" fill="url(#rcbad)" stroke="#4b5563" stroke-width="0.9"/>
<text x="622" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#374151" font-weight="700" text-anchor="middle">7</text>
<rect x="639.0" y="279.0" width="26" height="18" rx="3" fill="#ffffff" stroke="#9aa1ad" stroke-width="0.9"/>
<text x="652" y="291" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">8</text>
<rect x="669.0" y="279.0" width="26" height="18" rx="3" fill="#ccfbf1" stroke="#0d9488" stroke-width="1.2"/>
<text x="682" y="291.5" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8.5" fill="#0d9488" font-weight="700" text-anchor="middle">D</text>
<rect x="726" y="232" width="176" height="78" rx="5" fill="#fdf6e3" stroke="#d97706" stroke-width="1.1"/>
<path d="M754 257 L874 257" stroke="#9aa1ad" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>
<path d="
" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M298 1256 L298 1225 L334 1225 L334 1256 L490 1256 L490 1225 L526 1225 L526 1256 L682 1256 L682 1225 L718 1225 L718 1256 L860.3 1256" stroke="#0d9488" stroke-width="1.3" fill="none" stroke-opacity="1.0" stroke-dasharray="3 2" marker-end="url(#rcac)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 205 L874 225.0 L754 225.0 L754 283 L860.3 283" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 297 L874 317.0 L754 317.0 L754 375 L860.3 375" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 389 L874 409.0 L754 409.0 L754 467 L860.3 467" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 481 L874 513.0 L754 513.0 L754 583 L860.3 583" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 597 L874 617.0 L754 617.0 L754 675 L860.3 675" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 689 L874 709.0 L754 709.0 L754 767 L860.3 767" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 781 L874 801.0 L754 801.0 L754 859 L860.3 859" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 873 L874 905.0 L754 905.0 L754 975 L860.3 975" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 989 L874 1009.0 L754 1009.0 L754 1067 L860.3 1067" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 1081 L874 1101.0 L754 1101.0 L754 1159 L860.3 1159" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 1173 L874 1193.0 L754 1193.0 L754 1251 L860.3 1251" stroke="#7c3aed" stroke-width="2.6" fill="none" stroke-opacity="0.6" marker-end="url(#rcap)" stroke-linejoin="round" stroke-linecap="round"/>
<text x="746.0" y="228.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">链首，直接发出</text>
<text x="746.0" y="320.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">落本组，加后发出</text>
<text x="746.0" y="412.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">不落，只转发</text>
<text x="746.0" y="516.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">不落，只转发</text>
<text x="746.0" y="620.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">落本组，加后发出</text>
...
<text x="746.0" y="804.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">不落，只转发</text>
<text x="746.0" y="908.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">不落，只转发</text>
<text x="746.0" y="1012.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">不落，只转发</text>
<text x="746.0" y="1104.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">不落，只转发</text>
<text x="746.0" y="1196.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">落本组，加后发出</text>
<text x="746.0" y="1292.0" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#7c3aed" font-weight="700" text-anchor="end">落本组，加完出核</text>
...
<text x="942" y="333" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">落本组 → 进 Matrix Mem，等与本行</text>
...
<text x="942" y="363" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">不落本组 → 不相加，只转发上一行的结果</text>
<text x="942" y="378" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">该用户的第一行：本行结果直接发出</text>
<text x="942" y="393" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">链尾（第 11 行）：经 core4 从东口出核</text>
...
<text x="942" y="470" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">reduce 数据（青）：本行 dot core 沿行链</text>
<text x="942" y="485" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">　逐跳 reduce 出来的本行结果</text>
<text x="942" y="500" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">reduction 数据（紫）：沿链在行之间</text>
...
<rect x="930" y="568" width="250" height="188" rx="6" fill="#ffffff" stroke="#5c6370" stroke-width="1.3"/>
...
<text x="942" y="606" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">紫粗线：reduction 数据逐行往下走：</text>
<text x="942" y="621" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">　上一行 R core 南口 → 本行北口 core0</text>
<text x="942" y="636" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">　→ 经 mid 到 core5 → 沿下排落到 R core</text>
<text x="942" y="651" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">青虚线：行链，从第一列的 dot core 起，</text>
<text x="942" y="666" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">　逐跳 reduce 落进本行 R core</text>
<text x="942" y="681" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">B：B core，组头 chip 的 core0</text>
<text x="942" y="696" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">R：R core，最后一列 chip 的 core9</text>
<text x="942" y="711" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">D：dot core，每颗 chip 的逻辑 core7</text>
<text x="942" y="726" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">斜线格：坏 core，只有 Router 透传</text>
<text x="942" y="741" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#5c6370" font-weight="400" text-anchor="start">虚线格：不派角色的好 core，只转发</text>
<path d="M902 1270 L916 1270 L916 208 L929 208" stroke="#d97706" stroke-width="1.4" fill="none" stroke-opacity="1.0" stroke-dasharray="5 3" marker-end="url(#rcar)" stroke-linejoin="round" stroke-linecap="round"/>
...
<path d="M480 1338 L440.7 1338" stroke="#6b7280" stroke-width="1.6" fill="none" marker-end="url(#rca)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M300 1338 L260.7 1338" stroke="#6b7280" stroke-width="1.6" fill="none" marker-end="url(#rca)" stroke-linejoin="round" stroke-linecap="round"/>
<path d="M874 1247 L874 1225 L928 1225 L928 1338 L620.7 1338" stroke="#d97706" stroke-width="2.0" fill="none" marker-end="url(#rcar)" stroke-linejoin="round" stroke-linecap="round"/>
<text x="934" y="1221" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9" fill="#d97706" font-weight="400" text-anchor="start">东口出核</text>
...
<rect x="30" y="1386" width="1140" height="110" rx="5" fill="#f5f6f8" stroke="#9aa1ad" stroke-width="1.2"/>
<text x="46" y="1403" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">全图 3 tray × 4 层 × 4 chip = 48 chip，每颗 chip 2×5，core 按 row-major 编号，Router 居中；黄底是该用户有激活专家的组，灰底是不落的组。</text>
<text x="46" y="1419" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">中间两列 chip 的 core2、core7 是坏 core（斜线格），只有 Router 透传；虚线格是不派角色的好 core，只转发。四个 chip 口是行 0 左端 = 北口、行 0 右端 = 东口、行 1 左端 = 西口、行 1 右端 = 南口。</text>
<text x="46" y="1435" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">每行一个 R core，12 个 R core 逐行串成一条链；本行 4 颗 chip 的 dot core 沿行链逐跳 reduce 进本行 R core。紫字是每个 R core 的动作：链首直接发出、落本组加后发出、不落只转发。</text>
<text x="46" y="1451" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">串成链换来的是出口带宽与 EP 组数解耦：各组各自出核要 60 GB/s × 6 = 360 GB/s、9 个 ETH 口；串成链只有链尾一处 60 GB/s、2 个口。每一跳搬的都是同一份累加中间结果，加 EP 组不会让任何一跳变宽。</text>
<text x="46" y="1467" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">代价是整条链成了一个整体：任何一个 R core 把 reduce-buffer 占满，链上游全部停住，所以派遣必须保守，用户不落的组也要确认余量。</text>
<text x="46" y="1483" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="10.5" fill="#5c6370" font-weight="400" text-anchor="start">与第二层的差别：这里每一跳要等两笔齐了再加（用户之间乱序），不是流着加；缓冲是 32 MB 的 Matrix Mem，不是 ReduceModule 的 16 × 32 KiB。</text>
</svg>
```

R core 每行一个，是最后一列每颗 chip 的 `core9`，也就是那颗 chip 的南口；LPU 12 行共 12 个。R core 不分配权重，不参与 TP 与 FFN 计算。12 个 R core 逐行串成一条链：第 0 行 R core 是链首，每行 R core 把本行结果与上一行 R core 送来的累加结果相加，送给下一行；第 11 行 R core 的结果经 mid 到 core4，从右下角 chip 的东口经 PCIe Switch 出核，一个用户只出核一次。

R core 之间每行一跳：从本行 R core 的南口下到下一层最后一列 chip 的北口，经 core0、core5、core6、core7、core8 落到 core9。

这条通路上有两种数据，名字相近但不是一回事：

* **reduce 数据**：本行 4 颗 chip 的 dot core 沿行链逐跳 reduce 出来的本行结果，从同一颗 chip 的 dot core 进来
* **reduction 数据**：沿链在行之间流动的累加中间结果，从上一行的 R core 进来

每一跳只有两种动作：

1. R core 的 Router 收下 dot core 送来的 reduce 数据，存进本 core 的 Matrix Mem
2. 本行是该用户的第一行：累加后经 reduction 通路发往下一行 R core
3. reduction 数据到达 R core 后判断该用户是否落在本行所在的 EP 组：**落在本组就进 Matrix Mem，等与本行的 reduce 数据累加后再发出；不落在本组就不相加，只转发上一行的结果**
...
每一跳搬的都是同一份累加中间结果，加 EP 组不会让任何一跳变宽。代价是整条链成了一个整体：任何一个 R core 把 reduce-buffer 占满，链上游全部停住。
...
<text x="48" y="131" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">① 本行的 reduce 数据（dot core 送来）</text>
<text x="48" y="143" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="start">② 上一行 R core 的 reduction 数据</text>
...
<rect x="770" y="214" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="790" y="228" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R0: n</text>
<rect x="820" y="214" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="840" y="228" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R1: n</text>
<rect x="870" y="214" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="890" y="228" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R2: n</text>
<rect x="920" y="214" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="940" y="228" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R3: n</text>
<rect x="970" y="214" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="990" y="228" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R4: n</text>
<rect x="1020" y="214" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="1040" y="228" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R5: n</text>
<rect x="770" y="240" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="790" y="254" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R6: n</text>
<rect x="820" y="240" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="840" y="254" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R7: n</text>
<rect x="870" y="240" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="890" y="254" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R8: n</text>
<rect x="920" y="240" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="940" y="254" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R9: n</text>
<rect x="970" y="240" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="990" y="254" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R10: n</text>
<rect x="1020" y="240" width="40" height="20" rx="3" fill="#ffffff" stroke="#d97706" stroke-width="1.0"/>
<text x="1040" y="254" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="8" fill="#d97706" font-weight="600" text-anchor="middle">R11: n</text>
<text x="910" y="280" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.5" fill="#5c6370" font-weight="400" text-anchor="middle">dispatcher 里每个 R core 一个余量计数，12 行共 12 个</text>
...
<text x="40" y="400" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="9.5" fill="#16181d" font-weight="700" text-anchor="start">R core 链</text>
...
<text x="560" y="286" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="6.5" fill="#5c6370" font-weight="400" text-anchor="middle">BF16 进出，中间累加 FP32</text>
...
<text x="690" y="226" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="6.5" fill="#5c6370" font-weight="400" text-anchor="middle">dot core 做完广播给另外 7 个 core</text>
...
<text x="950" y="211" font-family="'Noto Sans CJK SC', 'PingFang SC', 'Microsoft YaHei', -apple-system, sans-serif" font-size="7.8" fill="#0d9488" font-weight="700" text-anchor="middle">⑥ 在 dot core 上 concat</text>
...
3. FC1 / FC3：chip 内切 K，每个 core 一份部分和；**第二层**沿 chip 内归约链把它们逐跳 reduce 汇聚到 dot core
4. dot core 做 SiLU · dot · 量化，再把 FC2 输入广播给本 chip 另外 7 个计算 core
