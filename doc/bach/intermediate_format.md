...
UTF-8 编码，LF 换行。一行一条记录，行首是记录标签，其后是字段，字段之间用一个或多个空格分隔。
...
数值一律十进制整数，可带负号。格式里没有浮点：Bach 的仿真时间轴上只出现非负整数，字节数、拍数与延迟都是整数，MoE 的权重与 softmax 不进入这份文件，因为随机决策已经在生成侧算完。
...
只读的来源信息，不影响模型行为。解析器把它原样存下，写进观测产物的 provenance。
...
覆盖参数表里的同名默认值。名字用 `common/params.h` 里的字段名，例如 `dte_setup_time`、`stream_count`、`noc_bandwidth`。
...
恰好一条。六个值都是正整数，`node_chip_rows` 整除 `chip_rows`，`node_chip_cols` 整除 `chip_cols`。
...
`group_id` 是该核所属的 MoE EPGroup id，非 MoE Map 写 `-1`。

`core_type` 取 `NORMAL`、`BROADCAST`、`REDUCTION` 之一，它决定上游给该核开的 credit 额度，以及直接驱动它的 Host 的 credit 容量。
...
路由完全看 `dst_row` 与 `dst_col`：目标坐标落在核阵列内就是某个 Core，落在核阵列外就是某个外部节点。`down_cid` 只用于 credit 归属与诊断。

`up_cid` 与 `down_cid` 在与它们无关的行上仍会带一个值，解析器不读那些位置。`RETIRE` 行的 `up_cid` 为 `-1` 时，上游是 Host，`dst_row` 与 `dst_col` 就是该 Host 的坐标。
...
该 `CU` 行要一次性验资的全部下游，至少一个。所有下游的额度同时到位才放行，任何一个堵住整条任务就卡住。
...
软件 credit 图的一条边：`src_core` 向 `dst_core` 申请与归还额度，额度上限由 `dst_core_type` 决定。`dst_core_type` 取 `NORMAL`、`BROADCAST`、`REDUCTION` 之一。

这张图与物理连线、与任务表都不互相推导，所以单独给。`CREDIT` 记录说的是"某一条验资任务要验哪几个下游"，不说明开了哪些户：有的核一条验资任务都没有，却仍要接收下游退休时还回来的额度。
...
每条屏障 `SKIP` 行的来源。`group_id` 列表是沿入边树递归收集的上游 group，可能为空。运行时用它判断这条入边在本次 HitMap 下会不会真的来包，不会来就自 ack 放行。
...
端口名取 `LOCAL`、`NORTH`、`EAST`、`SOUTH`、`WEST`、`PCIE_UP`、`PCIE_DOWN`、`PCIE_NORTH`、`PCIE_SOUTH`、`PCIE_EAST`、`PCIE_WEST`、`UNKNOWN` 之一。
...
`chip_rules` 解析后的出口网关，`direction` 取 `LEFT`、`RIGHT`、`TOP`、`BOTTOM`。`local_core_id` 是片内一维 core id，可以有多个。该方向没有网关时不写这条记录。
...
一条片间 PCIe 链路的一个方向：从 `src_core` 的 `src_port` 出去，到 `dst_core` 的 `dst_port`。`bandwidth` 与 `delay` 是发送侧那个端口上的取值，`bandwidth` 为 `0` 表示用默认的 PCIe 带宽。

每条物理链路写两条记录，两个方向各一条，因为两端的带宽与延迟取自各自 chip 的方向规则，可以不对称。

接线关系已经在这里算完，解析侧照表连，不从 `GATEWAY` 与尺寸去推导。`GATEWAY` 供跨 chip 路由时选本 chip 的出口网关用，与物理链路是两件事。
...
`PCIESW` 的 `chip` 是绑定的 chip 序号，未绑定写 `-1`；`row` 与 `col` 是不与核阵列重叠的锚点坐标。同一个 `sw_id` 只能出现一次。

`PCIELINK` 的端点 kind 取 `SW`、`CORE`、`HOST`、`OUT`，一条链路至少有一端是 `SW`。`a_ref` 按 kind 取值：`SW` 是交换节点 id，`CORE` 是 core id，`HOST` 与 `OUT` 是 `EXTNODE` 的 name。
...
`PCIEROUTE` 与 `PCIEIROUTE` 里的 `out_port` 与 `in_port` 都是交换节点自己的端口名。两张表里 `PCIEIROUTE` 优先于 `PCIEROUTE`。有 `PCIELINK` 就必须有其中至少一张表的记录：交换节点没有默认路由。

`HOSTGROUP` 给出每个 Host 的每个 EPGroup 应该注入哪个 Core。没有 Phase 时每个 user 实际注入哪个核由 `USERTARGET` 逐条给出，装配只读那一张；开着 Phase 的注入时读的是这一张，因为那时一个 user 进哪个核要等交换节点把专家换算成 EPGroup 之后才知道。一个 Host 只有一个注入目标，所以它名下那几个 group 都指向同一个核。

出现在 `PCIELINK` 里的 `HOST` 与 `OUT` 不再由 `EXTNODE` 的 `port` 直接接到网关核上，它进出阵列的那条线由这条 `PCIELINK` 给出。`EXTNODE` 的 `port` 此时仍然有用：跨 chip 路由要靠它把这个外部节点换算成网关核。
...
`PHASE1SINK` 的 `role` 取 `GENERIC`、`MOE`、`BYPASS` 之一，它决定这个落点把收齐的包交到以太网交换节点的哪个入口：`MOE` 交到 `phase1_moe_ingress`，`BYPASS` 交到 `res_ingress`，`GENERIC` 不往下交。
...
两路不能落在同一处：它们在下一跳走的是交换节点的两条不同入口。两路都写减号也不行，那条通道什么也不发。

`PHASE1HIT` 给出这条通道选中的那几个专家，交换节点按它换算成 EPGroup。要发 MoE 请求就必须有这一条。

`PHASE1USER` 给出这条通道负责哪些 uid，顺序就是推进去的顺序。各条通道的 uid 集合互不重叠。
...
`ETHSW` 至多一条。`header_bytes` 加 `payload_numel` 乘 `payload_bytes_per_elem` 就是一包的字节数，交换节点上的串行时间按它算。`expert_count` 写 `0` 表示不限。
...
`bandwidth` 的单位是字节每拍。`queue_cap` 与 `pending_cap` 是出口上排队的两级深度，两级都满就报错，写 `0` 表示用默认值。

`ETHROUTE` 的起点必须是入口、终点必须是出口，一条入口只能有一条出路。至少有一条。

`EXPGROUP` 给出一个专家落在哪几个 EPGroup 上，不写这张表时按 `expert_group_size` 整除。同一个专家只能写一条。
...
`result_bridge` 开着时另外三条同时成立：`phase2_ingress` 与 `inject_host` 必须开着，`boundary` 必须关着，`EXTNODE` 里 `kind` 为 `OUT` 的恰好一个。全局完成权只能有一处，`boundary` 与汇合点是同一件事的两种判法。

`phase2_ingress` 开着时要有 `HOSTBIND` 说明哪个 EPGroup 归哪个注入源，要有 `HOSTGROUP` 说明推到哪个核。
...
它与完成判据是两回事：一个 user 的结果可以落在几个汇聚点上，收到第一个就算它完成了，余下那些仍然会来，只是不再改结论。判完成的是以太网交换节点时这个数没人读，写 `0`。
...
每个 uid 一条，给出它由谁注入。`src_kind` 取 `HOST`、`DISPATCHER`、`PHASE1_LANE`。三类注入源产生的 uid 集合互不重叠。
...
该 uid 要从哪个 Host 发往哪个 Core，`tag` 是该 Host 上的命中次数之和。一个 uid 可以有多条，对应 MoE 分发到多个 Host 与目标的情形。非 MoE 的 uid 恰好一条。
...
注入源开跑的 uid 恒为 `1`：它的结果可以落在几个汇聚点上，但注入源不数它们，收到第一个就算这个 user 完成了。MoE 的分发器开跑的 uid 按分发结果决定：timeline 里有 `REDUCTION` 时取 Out 的个数，否则取本次命中的 EPGroup 个数。
...
VectorCore 在 `router_softmax_topk` 完成那一刻生成的 HitMap，按 uid、层号与核 id 索引。内容预先算好，可见时刻仍由模型控制：它要等那次计算结束后才对下游可见。
...
除首行版本行外，记录之间顺序无关。解析分两遍：第一遍读入全部记录，第二遍做校验。

### 7.2 遇到不认识的就停

未定义的记录标签、未定义的 `PARAM` 名字、未定义的 `TMETA` key、未定义的枚举取值，一律报错退出。`META` 的 key 是唯一的例外。

字段数不符、非整数字段解析失败、版本号不匹配，同样报错退出。

### 7.3 必需记录

`DIM` 恰好一条，`TOTALUSERS` 恰好一条，至少一条 `CORE`，至少一条 `EXTNODE`，至少一条 `USER`。
...
- 每条 `PCIELINK` 至少一端是 `SW`；`SW` 端点指向已声明的 `PCIESW`，`CORE` 端点的 core id 落在核阵列内且写了端口，`HOST` 与 `OUT` 端点指向 `kind` 相符的 `EXTNODE`。
...
端口有没有真接过线，以及一条路由的出口落在哪，要等装配期拿到实际拓扑才查得了，不在这一层。

***

## 8. 一个最小完整例子

一个 chip、两个 Core、一个 Host、一个 Out、一个 user 的 dense 拓扑。Core 0 收 Host 的包，算一次 Matrix，验资后发给 Core 1；Core 1 搬给 Out；两个核各自退休。
